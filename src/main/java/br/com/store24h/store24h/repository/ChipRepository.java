...
extends JpaRepository<ChipModel, Long> {
    ...
    @Query(value="SELECT c FROM ChipModel c WHERE c.alugado = :alugado AND c.ativo = :ativo AND c.operadora = :operator ORDER BY RAND()  limit :limite", nativeQuery=true)
    ...
    @Query(value="SELECT c FROM ChipModel c WHERE c.alugado = :alugado AND c.ativo = :ativo ORDER BY RAND() ", nativeQuery=true)
